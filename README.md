...
- **视频广告检测（v2.9.0+）**：自动下载群内视频（受体积/超时上限保护），用 OpenCV 等间隔抽帧，逐帧走视觉模型识别 + 二维码解码，识别文本并入正文走统一「初筛 + LLM 二次判断 + 处罚」流程；识别失败静默降级不误杀（默认关闭，可按群覆盖）
...
| `ad_backend_token`          | 独立后台访问令牌（留空不鉴权）        | 空          |
